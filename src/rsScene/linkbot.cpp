...
void Linkbot::draw(Group *group, const rs::Pos &p, const rs::Quat &q, const rs::Vec &a, const rs::Vec &c, bool trace) {
	...
	// draw body
	body[Bodies::Body] = osgDB::readNodeFile(_model_path + "linkbot/body.3ds");
	...
	pat[Bodies::Cap1]->setPosition(osg::Vec3d(p1[0], p1[1], p1[2]));
	...
	if (this->getForm() == rs::LinkbotI) {
		...
		body[Bodies::Cap2]->getOrCreateStateSet()->setAttribute(create_material(osg::Vec4(0.867, 0.827, 0.776, 1)));
	}
	else {
		body[Bodies::Cap2] = osgDB::readNodeFile(_model_path + "linkbot/face_rotate.3ds");
		body[Bodies::Cap2]->getOrCreateStateSet()->setAttribute(create_material(osg::Vec4(0, 0, 0, 1)));
	}
	pat[Bodies::Cap2]->setPosition(osg::Vec3d(p1[0], p1[1], p1[2]));
	...
	if (this->getForm() == rs::LinkbotL) {
		...
		body[Bodies::Cap3]->getOrCreateStateSet()->setAttribute(create_material(osg::Vec4(0.867, 0.827, 0.776, 1)));
	}
	else {
		body[Bodies::Cap3] = osgDB::readNodeFile(_model_path + "linkbot/face_rotate.3ds");
		body[Bodies::Cap3]->getOrCreateStateSet()->setAttribute(create_material(osg::Vec4(0, 0, 0, 1)));
	}
	pat[Bodies::Cap3]->setPosition(osg::Vec3d(p1[0], p1[1], p1[2]));
	pat[Bodies::Cap3]->setAttitude(osg::Quat(q1[0], q1[1], q1[2], q1[3]));
...
void Linkbot::drawConnector(Group *group, int type, int face, int orientation, double size, int side, int conn) {
	...
	switch (type) {
		...
		case Connectors::Wheel:
			node = osgDB::readNodeFile(_model_path + "linkbot/tinywheel.3ds");
			transform->setScale(osg::Vec3d(1, this->getWheelRatio(Connectors::TinyWheel), this->getWheelRatio(Connectors::TinyWheel)));
			break;
	}
	node->computeBound();
