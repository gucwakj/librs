...
MiniDof::MiniDof(int form) : rsRobots::Robot(form), rsRobots::MiniDof(form), rsScene::Robot() { }
...
/**********************************************************
	public functions
 **********************************************************/
void MiniDof::draw(Group *group, const rs::Pos &p, const rs::Quat &q, const rs::Vec &a, const rs::Vec &c, bool trace) {
	// initialize variables
	osg::ref_ptr<osg::Node> body[Bodies::Num_Parts];
	osg::ref_ptr<osg::PositionAttitudeTransform> pat[Bodies::Num_Parts];
	short enabled = dynamic_cast<rsRobots::MiniDof *>(this)->getEnabled();
	float scale = dynamic_cast<rsRobots::MiniDof *>(this)->getScale();
	...
	pat[Bodies::Body]->setScale(osg::Vec3d(scale, scale, scale));
	...
	pat[Bodies::Cap]->setScale(osg::Vec3d(scale, scale, scale));
...
void MiniDof::drawConnector(Group *group, int type, int face, int orientation, double size, int side, int conn) {
	// get robot p&q
