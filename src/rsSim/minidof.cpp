...
MiniDof::MiniDof(short joint) : rsRobots::Robot(rs::MiniDof), rsRobots::MiniDof(joint) {
...
/**********************************************************
	public functions
 **********************************************************/
int MiniDof::addConnector(int type, int face, int orientation, double size, int side, int conn) {
...
const rs::Vec MiniDof::getJoints(void) {
	return rs::Vec(_motor[Bodies::Joint].theta);
}

#ifdef RS_RESEARCH
void MiniDof::moveJointSingular(bool wait) {
...
/**********************************************************
	protected functions
 **********************************************************/
int MiniDof::build(const rs::Pos &p, const rs::Quat &q, const rs::Vec &a, const rs::Vec &w, int ground) {
	// build
...
int MiniDof::build(const rs::Pos &p, const rs::Quat &q, const rs::Vec &a, dBodyID base, int face, int orientation, int ground) {
...
void MiniDof::simPreCollisionThread(void) {
	// lock angle and goal
...
void MiniDof::simPostCollisionThread(void) {
	// lock angle and goal
	MUTEX_LOCK(&_goal_mutex);
	MUTEX_LOCK(&_theta_mutex);
...
/**********************************************************
	private functions
 **********************************************************/
void MiniDof::build_body(const rs::Pos &p, const rs::Quat &q) {
	// set mass of body
	dMass m, m1;
...
void MiniDof::build_cap(const rs::Pos &p, const rs::Quat &q) {
	// set mass of body
	dMass m;
...
void MiniDof::build_el(Connector &conn) {
	// set mass of body
	dMass m;
...
void MiniDof::build_foot(Connector &conn) {
	// set mass of body
	dMass m;
	dMassSetBox(&m, 170, this->getConnDepth(), 2*this->getCapRadius(), this->getConnHeight());
	dMassTranslate(&m, -m.c[0], -m.c[1], -m.c[2]);
...
void MiniDof::build_plank(Connector &conn) {
	// set mass of body
	dMass m;
...
void MiniDof::build_robot(const rs::Pos &p, const rs::Quat &q, const rs::Vec &a) {
	// init body parts
